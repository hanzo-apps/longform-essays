 ...
 * default so the app runs against live Hanzo (hanzo.id + api.hanzo.ai) with zero
 * config; the deploy pipeline overrides them per environment.
 */
...
/**
 * IAM application (client) id, `<org>-<app>`. Defaults to the shared `hanzo-app`
 * client; the deploy provisions a per-app client and injects it as
 * VITE_IAM_CLIENT_ID.
 */
